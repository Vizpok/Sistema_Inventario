...
$productos = db()->select("SELECT p.ID_PRODUCTO, p.NOMBRE, p.SKU, c.NOMBRE AS CATEGORIA, i.CANTIDAD_TOTAL, i.CANTIDAD_DISPONIBLE, l.CODIGO_UBICACION FROM productos p LEFT JOIN categorias c ON p.ID_CATEGORIA = c.ID_CATEGORIA LEFT JOIN inventario i ON p.ID_PRODUCTO = i.ID_PRODUCTO LEFT JOIN ubicaciones l ON i.ID_UBICACION = l.ID_UBICACION $where ORDER BY p.NOMBRE ASC");
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?= number_format($producto['CANTIDAD_TOTAL']) ?></td>
                    <td><?= number_format($producto['CANTIDAD_DISPONIBLE']) ?></td>
                    <td><?= $producto['CODIGO_UBICACION'] ?? '-' ?></td>
                    <td>
                        <a href="inventario_editar.php?id=<?= $producto['ID_PRODUCTO'] ?>" class="btn-primary" style="padding:6px 12px; font-size:12px; border-radius:6px;">
